...
export class PathBuilder {
  ...
  addParametricPathDifferentiable(
    f: DifferentiableParametricFunction,
    ...
    const samples = initializedArray(numberOfSegments + 1, (index) => {
      ...
      const point = { x, y };
      const direction =
        dx === 0 && dy === 0 ? NaN : Math.atan2(dy, dx);
...
export type PointWithDerivative = Point & { dx: number; dy: number };
export type DifferentiableParametricFunction = (t: number) => PointWithDerivative;
...
export class PathShape {
   ...
   * @deprecated It's better to call {@link fromJson} or {@link fromRawString} directly.
   */
   ...
   * @throws An invalid input will throw a PathShapeError.
   */
  static fromRawString(d: string): PathShape {
   ...
   *
   * See also {@link fromString}.  That works with JSON and other formats.
   ...
   */
  static parametricDifferentiable(
    f: DifferentiableParametricFunction,
    numberOfSegments: number,
  ): PathShape {
    const start = f(0);
    return PathBuilder.M(start.x, start.y).addParametricPathDifferentiable(
      f,
      numberOfSegments,
    ).pathShape;
  }
  ...
  static glitchFreeParametricDifferentiable(
    f: DifferentiableParametricFunction,
    initialSegments: number,
  ): PathShape {
    const firstTry = this.parametricDifferentiable(f, initialSegments);
    const result = this.#glitchFreeParametric(f, initialSegments, 0, firstTry);
    return assertNonNullable(result);
  }
  ...
  static #glitchFreeParametric(
    ...
    const firstTry = firstTryOverride ?? this.parametric(f, initialSegments);
