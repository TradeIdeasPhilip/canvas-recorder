...
import {
  Command,
  DifferentiableParametricFunction,
  LCommand,
  ParametricFunction,
  PathShape,
  Point,
...
export function termsToParametricFunction(
   ...
   * The main event!
   *
   * Compute each of the terms and add the results, plus the analytic derivative.
   * @param t A value from 0 to 1, inclusive.
   * @returns A sum of all of the terms computed at `t`, with derivative.
   */
  function sumOfFourierTerms(t: number): PointWithDerivative {
    let x = 0,
      y = 0,
      dx = 0,
      dy = 0;
    for (let k = start; k < end; k++) {
      ...
      x += amplitude * cosAngle;
      y += amplitude * sinAngle;
      const twoPiFreq = 2 * Math.PI * frequency;
      dx -= amplitude * twoPiFreq * sinAngle;
      dy += amplitude * twoPiFreq * cosAngle;
    }
    return { x, y, dx, dy };
  }
  /**
   ...
   * It helps a lot.
   */
  const cache = new Map<number, PointWithDerivative>();
  function cachedSumOfFourierTerms(t: number): PointWithDerivative {
  ...
  const result: ((t: number) => PathShape)[] = initializedArray(
    ...
    (index) => {
      ...
      if (
        ...
        const path = PathShape.glitchFreeParametricDifferentiable(
          parametricFunction,
        ...
        if (
          ...
          return (timeInMs: number): PathShape => {
            ...
            if (safePartEnds <= 0) {
              ...
              function parametricFunction(t: number): PointWithDerivative {
                const tScaled = t * safePartEnds;
                const base = startingFunction(tScaled);
                const fraction = 1 - getFraction(tScaled);
                if (fraction == 0) {
                  return {
                    x: base.x,
                    y: base.y,
                    dx: base.dx * safePartEnds,
                    dy: base.dy * safePartEnds,
                  };
                } else {
                  const adding = addingFunction(tScaled);
                  return {
                    x: base.x + fraction * adding.x,
                    y: base.y + fraction * adding.y,
                    dx: (base.dx + fraction * adding.dx) * safePartEnds,
                    dy: (base.dy + fraction * adding.dy) * safePartEnds,
                  };
                }
              }
              const path = PathShape.glitchFreeParametricDifferentiable(
            ...
            function parametricFunction(t: number): PointWithDerivative {
              ...
              if (fraction == 0) {
                ...
                return {
                  x: base.x + fraction * adding.x,
                  y: base.y + fraction * adding.y,
                  dx: base.dx + fraction * adding.dx,
                  dy: base.dy + fraction * adding.dy,
                };
              }
            }
            const path = PathShape.glitchFreeParametricDifferentiable(
...
export function createFourierAnimation(
  ...
  function getInfo(timeInMs: number) {
    ...
    if (unitized <= 0) {
      index = 0;
      progress = 0;
    } else if (unitized >= animationRules.length) {
      index = animationRules.length - 1;
      progress = 1;
    } else {
